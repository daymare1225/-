    ...
    unsafe {
        let option_type = match _type {
            ...
            CGEventType::KeyDown => {
                ...
                // The application only needs physical key state. Avoid expensive
                // text conversion for every macOS key-repeat event.
                let key = key_from_code_or_name(code.try_into().ok()?, None);
